    ...
    public class ApplicationHostService : IHostedService
    {
        ...
        private void OnNavigationWindowLoaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                navigationWindow.NavigationView.Navigate(typeof(GridOVTsPage)); // тут стартовая страница меняется
            }

            catch (Exception ex)
            {
                if (sender is MainWindow navigationWindow)
                {
                    navigationWindow.NavigationView.Navigate(typeof(SettingsPage));
                }
                ErrorHandler.ShowError(ex, "Ошибка загрузки приложения. Перейдите в настройки");
