...
{
    public class DocumentsSecondMap : IEntityTypeConfiguration<DocumentsSecondDb>
    {
        public void Configure(EntityTypeBuilder<DocumentsSecondDb> builder)
        {
            ...
            builder.HasMany(d => d.HardwaresDb).WithOne(h => h.DocumentSecondDb).HasForeignKey(h => h.DocLocationSecondId).OnDelete(DeleteBehavior.Restrict);


            builder.ToTable("Док. Второй");
        }
