...
{
    public class DocumentsThirdMap : IEntityTypeConfiguration<DocumentsThirdDb>
    {
        public void Configure(EntityTypeBuilder<DocumentsThirdDb> builder)
        {
            ...
            builder.HasMany(d => d.HardwaresDb).WithOne(h => h.DocumentThirdDb).HasForeignKey(h => h.DocLocationThirdId).OnDelete(DeleteBehavior.Restrict);


            builder.ToTable("Док. Третий");
        }
