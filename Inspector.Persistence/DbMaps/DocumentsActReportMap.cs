...
{
    public class DocumentsActReportMap : IEntityTypeConfiguration<DocumentsActReportDb>
    {
        public void Configure(EntityTypeBuilder<DocumentsActReportDb> builder)
        {
            ...
            builder.HasMany(d => d.CabinetsDb).WithOne(c => c.DocumentActReportDb).HasForeignKey(c => c.ActReportId).OnDelete(DeleteBehavior.Restrict);

            builder.ToTable("Акт обслед.");
        }
